...
- ⬜ Reconciler in the operator (opencode plugin poll loop or SDK script): watch Forgejo
  `ready` issues; hold `idle >= minAvailable`; claim an idle member per card → relabel
  `in-progress`.
- ⬜ Config: `minAvailable` (2–3), NodePort capacity ceiling, label scheme.
- ⬜ Execution-mode knob: human-supervised (default) vs. `--auto` unattended.
