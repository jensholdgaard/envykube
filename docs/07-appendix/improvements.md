...
11. **`grafana-token` race condition: health check passes before API is ready** — The `grafana-token` recipe (justfile:231) polls `GET /api/health` until it returns 200, but Grafana's health endpoint can respond 200 before the internal service account API is fully initialized. The subsequent curl to `/api/serviceaccounts/search` returns an empty body (or the connection resets), and the Python JSON parser at line 241 fails with `JSONDecodeError: Expecting value`. This causes the entire `just observability` recipe to fail at the final step — all backends deploy successfully but the `.env` token is never written. **Reproduction:** fresh cluster, first `just observability` run. **Workaround:** wait 30–60 seconds and re-run `just grafana-token`. **Fix:** replace the health-check loop with a readiness check against `/api/serviceaccounts/search` itself (retry until it returns valid JSON with a `serviceAccounts` key), or add a longer post-healthcheck sleep.
